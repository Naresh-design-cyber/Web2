...
(async () => {
  ...
  // Add the redirect route AFTER Vite setup to avoid conflicts
  app.get('/:shortCode', async (req, res, next) => {
    try {
      const { shortCode } = req.params;
      
      // Skip if this looks like a static file request - let Vite handle it
      if (shortCode.includes('.') || shortCode.startsWith('src/') || shortCode.startsWith('@')) {
        return next();
      }
      ...
      if (!url) {
        return next(); // Let Vite handle 404s for non-existent short codes
      }
